...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<AppState>(() => {
    const saved = localStorage.getItem('sprint_master_data');
    if (saved) {
      try {
        const data = JSON.parse(saved);
        // Migration: Ensure all cards have a code and comments array
        if (data.cards) {
          data.cards = data.cards.map((c: any, index: number) => ({
            ...c,
            code: c.code || `CRD-${(index + 1).toString().padStart(3, '0')}`,
            comments: c.comments || []
          }));
        }
        // Migration: Ensure users array exists and has the default admin if missing
  ...
  useEffect(() => {
    ...
    // Only save to localStorage if NO sync URL is configured
    if (!hasGasUrl) {
      localStorage.setItem('sprint_master_data', JSON.stringify(state));
    } else {
      // Clear local cache to force reliance on Cloud
      localStorage.removeItem('sprint_master_data');
    }
    
    // Auto-sync to GAS if URL is configured
    if (hasGasUrl) {
    ...
    async function pullData() {
      if (state.config.gasUrl && state.config.gasUrl.startsWith('http')) {
        try {
          ...
          if (data && data.config) {
            // Only overwrite if the pulled data actually has something 
            // OR if our current local state is effectively empty (except for admin)
