...
function MainLayout() {
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    const saved = localStorage.getItem('sprint_master_active_tab');
  ...
  const handleTabChange = (tab: Tab) => {
    setActiveTab(tab);
    localStorage.setItem('sprint_master_active_tab', tab);
  };
      ...
      >
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
           <div className="bg-slate-800/50 p-4 rounded-xl">
            ...
            <button 
              onClick={() => { handleTabChange('results'); calculateResults(); }} 
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative pt-16 md:pt-0">
        <header className="hidden md:flex h-16 bg-white border-b border-slate-200 px-8 items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
             <h1 className="text-lg font-bold text-slate-800 capitalize tracking-tight ring-slate-900">{activeTab === 'users' ? 'Gestão de Usuários' : activeTab === 'settings' ? 'Configurações' : activeTab === 'dashboard' ? 'Dashboard' : activeTab}</h1>
